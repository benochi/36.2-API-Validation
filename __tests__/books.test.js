...
beforeEach(async function(){
  ...
  VALUES
  ('111111111','https://amazon.com/book', 'Author', 'english', 2, 'Publisher', 'Title', 2021)
  RETURNING isbn`)
});
...
describe("POST /books", function(){
  test("Creates a book", async function(){
    const response = await request(app)
      .post(`/books`)
      .send({isbn: '111112112',
      amazon_url: "www.amazon.com/bob",
      author: "bob",
      language: "english",
      pages: 2,
      publisher: "Bobs dad",
      title: "I'm bob and I do things",
      year: 2021    
      });
  ...
  test("title required for books", async function(){
    const response = await (await request(app).post(`/books`)).send({author: "bobbert"});
